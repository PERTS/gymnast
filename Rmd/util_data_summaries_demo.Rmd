...
ds.build_glm1_formulas( dvs=dvs, ivs=ivs, mods=mods, cov_groups = cov_groups) %>%
    util.print_pre()

```

### Run and summarize with ds.glm1s() and ds.model_summary_df
This section models how to use `ds.glm1s()` and `ds.model_summary_df`.
...
summary_df <- ds.model_summary_df(models, summary_func=ds.summarize_glm1)
